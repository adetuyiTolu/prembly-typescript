...
import { Prembly, ClientOptions } from 'prembly';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Prembly)`
    : `const run: (${functionSource.client}: Prembly) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(['client.verification.verifyNin'], { threshold: 1, shouldSort: true });
